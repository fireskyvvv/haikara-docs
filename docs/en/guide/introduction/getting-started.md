...
詳細は[Create Binding](todo Url)を確認してください。
...
詳細は[HaikaraUI Attribute](todo Url)を参照してください。  
...
### 記載したパラメータの簡易的な説明
宣言した`BindableProperty`に使われているパラメータの簡易的な説明を列挙します。  
詳細は[BindableProperty](todo Url)を確認してください。  
...
上記の例では`FirstSampleViewModel.Label`を指定しています。  

- **elementNameInfo**
.uxml上のどのVisualElementに対してバインディングを構築するかを指定します。  
ここにはVisualElementの名前を文字列で指定できます。  
文字列を安全に指定できるようにするためにSourceGeneratorによって`ElementNames`という構造体が作成され、VisualElementの名前が宣言されています。  
上記の例では`ElementNames`を利用し`first-sample__label`を指定しています。  
詳細は、[View source generation](todo url)を確認してください。  
